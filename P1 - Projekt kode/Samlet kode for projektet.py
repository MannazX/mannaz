...
dataASDASD = np.genfromtxt('OpOgAfladning.csv', delimiter=',', skip_header = 2081)
def ladning(R = 2006, C = 100*10**-9, v0 = 1):
    ...
    oplad_afv      : Liste over afvigelsen for opladningen 
    aflad_afv      : Liste over afvigelsen for afladning
    ...
    for i in range(1760): 
        ...
        op_afv = (-2 * m.exp(-w0 * data[i,0]) * v0 + v0) - (data[i,2])
        opladning.append(y)
        oplad_afv.append(abs(op_afv))
    for i in range(1760): 
        y =  2 * m.exp(-w0 * tid[i]) * opladning[-1] - v0
        af_afv = (2 * m.exp(-w0 * data[i,0]) * 1 - v0) - (data[1761+i,2])
        afladning.append(y)
        aflad_afv.append(abs(af_afv))
    ...
    square, = plt.plot(data[:1760*2,0],data[:1760*2,1], label = 'Inputspænding', 
                       color = 'k', linewidth = 1)
    ...
    print('Afvigelser for ladningen V for simulation og Analog Discovery:\n' )
    print('Den maksimale afvigelse for opladning: {:.4f}\n'.format(max(oplad_afv)))
    print('Den maksimale afvigelse for afladning: {:.4f}\n'.format(max(aflad_afv)))
    print('Den gennemsnitlige afvigelse for opladning: {:.4f}\n'.format(sum(oplad_afv)/len(oplad_afv)))
    print('Den gennemsnitlige afvigelse for afladning: {:.4f}\n'.format(sum(aflad_afv)/len(aflad_afv)))
...
def lavpas_bode(R = 2006, C = 100*10**-9):
    ...
    mod_afv_list     : Liste for afvigelsen for længden af overføringsfunktionen.
    arg_afv_list     : Liste for afvigelsen for fasen af overføringsfunktionen.
    
    w0 = 1/(R*C*2*pi): Konstanten omega baseret på konstanterne for Resistans (R) og Kapacitans (C).
    ...
    """
    mod_list = []; arg_list =[]        
    mod_afv_list = []; arg_afv_list = []
    w0 = 1/(2*m.pi*R*C)
    for omega in range(len(data_mod[:,0])):
        modulus  =  w0/np.sqrt(data_mod[omega,0]**2+w0**2)    
        argument = np.arctan(-data_arg[omega,0]/w0) 
        mod_afv =  20*np.log10(modulus)-data_mod[omega,2]
        arg_afv  = 180/m.pi*(argument)-data_arg[omega,1]
 
        mod_afv_list.append(abs(mod_afv))
        arg_afv_list.append(abs(arg_afv))
...
#PLOTTER Overføringsfunktion
    ...
    inputtet, = plt.plot(data_mod[:,0],data_mod[:,1], color = 'k', label = 'Inputspænding')

    mod,= plt.plot(data_mod[:,0],mod_list, label = 'Spændingsforholdet', 
                   linestyle = "-", linewidth = 1)    
    AD, = plt.plot(data_mod[:,0],data_mod[:,2], label = 'Analog Discovery', 
                   linestyle = ":", linewidth = 3, color = 'y') 
    knæk_mod, = plt.plot(w0,knæk_mod, linewidth = 0, label = 'Knækfrekvens' ,marker = 'x',
                         markersize = 8, color = 'r')
    plt.xscale('log')
    plt.ylabel("Amplituden [dB]")
    plt.legend(handles = [AD, mod, inputtet, knæk_mod], loc = 'upper right')
    plt.grid(alpha = 0.5)
...
#PLOTTER Faseforskydning
    ...
    plt.xscale('log')
    plt.xlabel("frekvens [Hz]"); plt.ylabel("Fasen for vinklen $\Theta$ i grader")
    plt.legend(handles = [AD, arg, knæk_arg], loc = 'upper right')
    plt.grid(alpha = 0.5)

    print('Afvigelser for lavpassfilter:\n')
    print('Den gennemsnitlige afvigelse for længden af overføringsfunktionen: {:.4f}\n'.format(sum(mod_afv_list)/len(mod_afv_list)))
    print('Den maksimale afvigelse for længden af overføringsfunktionen: {:.4f}\n'.format(max(mod_afv_list)))
    print('Den gennemsnitlige afvigelse for Faseforskydingen: {:.4f}'.format(sum(arg_afv_list)/len(arg_afv_list)))
    print('Den maksimale afvigelse for Faseforskydingen: {:.4f}\n'.format(max(arg_afv_list)))    
...
def hojpas_bode(R = 2006, C = 100*10**-9):
    ...
    w0 = 1/(2*m.pi*R*C)
    for omega in range(len(data_mod[:,0])):
        modulus =  ((data_mod[omega,0]/w0)/np.sqrt(data_mod[omega,0]**2/w0**2+1))
...
#PLOTTER Faseforskydning
    ...
    print('Afvigelser for højpassfilter:\n')
    print('Den gennemsnitlige afvigelse for længden af overføringsfunktionen: {:.4f}\n'.format(sum(mod_afv_list)/len(mod_afv_list)))
    print('Den maksimale afvigelse for længden af overføringsfunktionen: {:.4f}\n'.format(max(mod_afv_list)))
    print('Den gennemsnitlige afvigelse for Faseforskydingen: {:.4f}'.format(sum(arg_afv_list)/len(arg_afv_list)))
    print('Den maksimale afvigelse for Faseforskydingen: {:.4f}\n'.format(max(arg_afv_list)))    
...
def pick_func():
    """
    Lader brugeren vælge funktionen som skal køre.    
    """
    print("Der kan vælges mellem tre funktioner:\n ladning()\n lavpas_bode()\n hojpas_bode()")
    var = str(input("Vælg venligst funktionen der skal køres:"))
    if var == "ladning()":
        ladning()
    elif var == "lavpas_bode()":
        lavpas_bode()
    elif var == "hojpas_bode()":
        hojpas_bode()
    else:
        print("der er ingen funktion kaldet:", var," \n kør koden igen")
pick_func()